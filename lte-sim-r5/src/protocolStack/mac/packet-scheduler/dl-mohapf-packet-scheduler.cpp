...
#include "dl-mohapf-packet-scheduler.h"
...
void
DL_MOHAPF_PacketScheduler::DoSchedule ()
{
...
double
DL_MOHAPF_PacketScheduler::ComputeSchedulingMetric (RadioBearer *bearer, double spectralEfficiency, int subChannel)
{
  /*
   * For the EXP scheduler the metric is computed
   * as follows:
   *                  targetDelay        -log(dropProbability)     availableRate
   * metric = exp( ------------------) * ---------------------- * ---------------
   *               targetDelay - HOL          targetDelay           averageRate
   */
